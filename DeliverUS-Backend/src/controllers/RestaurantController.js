...
const promote = async function (req, res) {
  try { 
    const promocionado = await Restaurant.findOne({
      where: {
        userId: req.user.id,
        promoted: true
      }
    ...
    const resturanteActualizado = await Restaurant.findByPk(req.params.restaurantId)
    res.json(resturanteActualizado)
  }  catch (err) {
    res.status(500).send(err)
  }
}
